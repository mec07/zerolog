...
func init() {
	...
	recursivelyIgnoredPkgs = append(recursivelyIgnoredPkgs, "github.com/mec07/zerolog")
	args := flag.Args()
	if len(args) != 1 {
...
func main() {
	...
	// get the github.com/mec07/zerolog.Event type
	event := getEvent(p)
	if event == nil {
		fmt.Fprintln(os.Stderr, "Error: github.com/mec07/zerolog.Event declaration not found, maybe zerolog is not imported in the scanned package?")
		os.Exit(1)
	}

	// get all selections (function calls) with the github.com/mec07/zerolog.Event (or pointer) receiver
...
func getEvent(p *loader.Program) types.Type {
	for _, pkg := range p.AllPackages {
		if strings.HasSuffix(pkg.Pkg.Path(), "github.com/mec07/zerolog") {
